...
const PLUGIN_NAME: &CStr = unsafe { CStr::from_bytes_with_nul_unchecked(b"OAUTHBEARER\0") };
const FORMAT: &CStr = unsafe { CStr::from_bytes_with_nul_unchecked(b"%s\0") };
const AUTHZ_NAME: &CStr = unsafe { CStr::from_bytes_with_nul_unchecked(b"Authorization Name\0") };
const AUTHZ_PROMPT: &CStr = unsafe {
    CStr::from_bytes_with_nul_unchecked(b"Please enter an authorization name\0")
};
const TOKEN_NAME: &CStr = unsafe { CStr::from_bytes_with_nul_unchecked(b"Access Token\0") };
const TOKEN_PROMPT: &CStr = unsafe {
    CStr::from_bytes_with_nul_unchecked(b"Please enter Access Token (as JWT)\0")
};
...
impl<'a> Utils<'a> {
    ...
    pub(crate) fn log(&self, level: i32, message: &str) {
        ...
        let log = match self.raw.log {
            Some(log) => log,
            None => return,
        };
        unsafe { log(self.raw.conn, level, FORMAT.as_ptr(), message.as_ptr()) };
    }
    ...
    pub(crate) fn set_error(&self, message: &str) {
        ...
        let seterror = match self.raw.seterror {
            Some(seterror) => seterror,
            None => return,
        };
        unsafe { seterror(self.raw.conn, 0, FORMAT.as_ptr(), message.as_ptr()) };
    }
    ...
    pub(crate) fn get_simple(&self, prompts: &Prompts, id: c_ulong) -> CbValue {
        ...
        let getcallback = match self.raw.getcallback {
            Some(getcallback) => getcallback,
            None => return CbValue::Fail(SASL_FAIL),
        };
    ...
    pub(crate) fn get_password(&self, prompts: &Prompts) -> CbValue {
        ...
        match unsafe { getcallback(self.raw.conn, SASL_CB_PASS, &mut proc_, &mut context) } {
            SASL_OK => {
                ...
                let bytes = unsafe {
                    let len = (*secret).len as usize;
                    let data: *const u8 = std::ptr::addr_of!((*secret).data).cast();
...
impl<'a> Prompts<'a> {
    ...
    pub(crate) fn request(&mut self, utils: &Utils, want_user: bool, want_pass: bool) -> i32 {
        ...
        if want_pass {
            push(SASL_CB_PASS, TOKEN_NAME, TOKEN_PROMPT);
        }
        push(SASL_CB_LIST_END, EMPTY, EMPTY);
