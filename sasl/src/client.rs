...
impl ClientStep<'_> {
    fn run(mut self) -> i32 {
        ...
        let anonymous = unsafe { CStr::from_bytes_with_nul_unchecked(b"anonymous\0") };
        let rc = canon.apply(anonymous, SASL_CU_AUTHID, &mut self.oparams);
        if rc != SASL_OK {
            return rc;
        }
        let rc = if let Some(authzid) = &authzid {
            let authzid_c = match CString::new(authzid.as_str()) {
                Ok(authzid_c) => authzid_c,
                Err(_) => {
...
unsafe extern "C" fn client_mech_step(
    ...
    guard(|| {
        ...
        let (utils, out, oparams) = match (
            ...
            (Some(utils), Some(out), Some(oparams)) => (utils, out, oparams),
            _ => return SASL_BADPARAM,
        };
...
pub unsafe extern "C" fn sasl_client_plug_init(
    ...
    guard(|| {
        ...
        let utils = match unsafe { Utils::from_raw(utils) } {
            Some(utils) => utils,
            None => return SASL_BADPARAM,
        };
