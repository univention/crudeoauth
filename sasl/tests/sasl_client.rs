...
const TOKEN_C: &[u8] = b"eyJhbGciOiJSUzI1NiJ9.eyJmYWtlIjoxfQ.c2lnbmF0dXJl\0";
const AUTHZID_C: &[u8] = b"u:someuser\0";
...
fn init() {
    INIT.call_once(|| unsafe {
        ...
        assert_eq!(
            sasl_client_add_plugin(b"oauthbearer\0".as_ptr() as *const c_char, Some(oauthbearer::sasl_client_plug_init)),
            SASL_OK
        );
    });
}
...
unsafe extern "C" fn cb_user(
    ...
    unsafe {
        *result = b"u:someuser\0".as_ptr() as *const c_char;
        if !len.is_null() {
...
fn secret_ptr() -> *mut sasl_secret_t {
    ...
    let bytes = size_of::<c_ulong>() + data.len() + 1;
...
unsafe fn new_conn(callbacks: *const sasl_callback_t) -> *mut sasl_conn_t {
    ...
    let rc = unsafe {
        sasl_client_new(b"ldap\0".as_ptr() as *const c_char, b"server.example.org\0".as_ptr() as *const c_char, null(), null(), callbacks, 0, &mut conn)
    };
...
fn client_emits_initial_response_from_callbacks() {
    ...
    unsafe {
        ...
        let rc = sasl_client_start(conn, b"OAUTHBEARER\0".as_ptr() as *const c_char, &mut prompts, &mut out, &mut outlen, &mut mech);
...
fn client_prompts_when_no_value_callbacks() {
    ...
    unsafe {
        ...
        while (*p).id != SASL_CB_LIST_END {
            match (*p).id {
                SASL_CB_USER => {
                    (*p).result = AUTHZID_C.as_ptr().cast();
                    (*p).len = AUTHZID.len() as c_uint;
                }
                SASL_CB_PASS => {
                    (*p).result = TOKEN_C.as_ptr().cast();
